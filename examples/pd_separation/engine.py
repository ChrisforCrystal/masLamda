...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        async def main_loop():
            # [核心逻辑] Leader 需要同时干两件事，这里用 asyncio 实现并发
            
            # 1. 启动后台调度循环（非阻塞）
            # create_task() 会立即返回，不等待 scheduler_loop() 完成
            # scheduler_loop() 会在后台死循环，每 10ms 醒一次去捞队列里的请求
            asyncio.create_task(scheduler_loop())
            
            # 2. 启动 HTTP 服务器（阻塞）
            # await server.serve() 会一直阻塞在这里，监听端口 8000/8001
            # 当有 HTTP 请求进来时，FastAPI 会唤起对应的 @app.post 处理函数
            # 那些处理函数会把请求扔进队列，然后 scheduler_loop 就能捞到了
            await server.serve()
            
            # 【重要】这两行代码的配合让 Leader 实现了"一心二用"：
            # - HTTP 线程：接收用户请求，扔进队列
            # - 调度线程：从队列捞请求，打包成 Batch，广播给 Worker
